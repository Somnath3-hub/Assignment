...
# Question 6 :  Create a database named ECommerceDB and perform the following tasks: 1. Create the following tables with appropriate data types and constraints: 2.  Insert the following records into each table 
# Ans :-
 ## Create Database
...
USE sakila;
SHOW TABLES;
# 1. Identify the top 5 customers based on total amount spent
USE sakila;
...
# 3. Calculate films available at each store and how many have never been rented
SELECT 
    i.store_id AS StoreID,
    COUNT(DISTINCT i.film_id) AS TotalFilmsAvailable,
    COUNT(DISTINCT CASE WHEN r.rental_id IS NULL THEN i.film_id END) AS NeverRentedFilms
FROM inventory i
LEFT JOIN rental r ON i.inventory_id = r.inventory_id
GROUP BY i.store_id
ORDER BY i.store_id;

# 4.Show total revenue per month for the year 2023
SELECT 
